...
public class HttpClient {
    ...
    /**
     * 发送 POST 请求（请求体为原始 JSON 字符串，不做二次序列化）
     * 供 KingdeeApiTool 使用，避免 AI 生成的 JSON 被二次转义
     *
     * @param url         请求地址
     * @param headers     请求头
     * @param rawJsonBody 已经序列化好的 JSON 字符串
     * @return HTTP 响应
     */
    public HttpResponse postRaw(String url, Map<String, String> headers, String rawJsonBody) {
        log.debug("POST(raw) {} 请求体长度: {}", url, rawJsonBody.length());
        RequestBody requestBody = RequestBody.create(rawJsonBody, JSON);
